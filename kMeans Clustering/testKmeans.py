import numpy, random, cluster
import matplotlib.pyplot as plt
class Patient(cluster.Example):
    pass

#Z-Scaling of the parameters part of Feature Engineering
def scaleAttrs(vals):
    vals = numpy.array(vals)
    mean = sum(vals)/len(vals)
    sd = numpy.std(vals)
    vals = vals - mean
    return vals/sd #This is the new feature value which is std normal variate

#toScale will call the Z-scaling
def getData(toScale = False):
    ...
    if toScale:
        ...
        #Each variable will become normally distributed
